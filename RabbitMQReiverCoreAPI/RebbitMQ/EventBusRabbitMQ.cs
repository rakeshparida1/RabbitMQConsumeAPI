        ...
        private void ReceivedEvent(object sender, BasicDeliverEventArgs e)
        {
            if (e.RoutingKey == "userInsertMsgQ")
            {
                ...
                using (var client = new HttpClient())
                {
                    try
                    {
                        client.BaseAddress = new Uri("http://localhost:802/");
                        //HTTP GET
                        var responseTask = client.PostAsJsonAsync("values/savedata", userList);
                        responseTask.Wait();

                        var result = responseTask.Result;
                        if (result.IsSuccessStatusCode)
                        {
                            Console.WriteLine("get ok api");
                            var readTask = result.Content.ReadAsStringAsync();

                            readTask.Wait();

                            var students = readTask.Result;
                            Console.WriteLine("get api response", students);
                        }
                    }
                    catch (Exception )
                    {
